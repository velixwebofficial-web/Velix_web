...
module.exports = async (req, res) => {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    // 503, not 500: this is "the server is up but a required service isn't
    // configured yet," not an actual server error.
    res.status(503).json({ error: 'AI is not configured on the server yet (missing ANTHROPIC_API_KEY).' });
    return;
  }
