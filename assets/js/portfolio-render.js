/* ==========================================================================
   ...
   Both read straight from VELIX.projects (local API-backed single source
   of truth in store.js) — nothing here is hardcoded, and nothing is capped:
...
(function () {
  ...
  function addressFor(p) {
    ...
    return `velixweb.xyz/work/${p.slug || p.id}`;
  }
  ...
  function projectCard(p, i) {
    const body = p.cover
      ? window.VELIX_IMG.wrapImgHTML(`<img src="${escapeAttr(p.cover)}" alt="${escapeAttr(p.title)}">`)
