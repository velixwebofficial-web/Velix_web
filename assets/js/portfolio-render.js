...
(function () {
  ...
  function init() {
    ...
    // Wait for the store's initial Supabase fetch to finish before the
    // first paint, otherwise we'd render against an empty cache.
    window.VELIX.ready.then(() => {
      renderFeaturedPreview();
      renderFullPortfolio();
    });
    // Supabase Realtime tells us the moment any visitor/admin changes a
    // project anywhere — repaint immediately so this is a live page, not
    // just a page that happens to be correct on first load.
