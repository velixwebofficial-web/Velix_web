...
1. `vercel env add ANTHROPIC_API_KEY` — paste your real Anthropic key. This
   is required for the AI chat widget to come alive.
2. Set `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` (from your Supabase
   project's API settings) as Vercel env vars — this is what makes leads,
   events, and everything else in `/lib/store.js` durable. See
   `SUPABASE_MIGRATION.md` for full setup. Without this, leads/events
   captured by the AI are not persisted server-side.
3. `git push` / `vercel --prod` as normal — `package.json` now declares
   `@anthropic-ai/sdk` and `@supabase/supabase-js`, Vercel installs them
...
Admin Integration (leads read live from Supabase in the admin Kanban) ·
...
Postgres Row Level Security) · Performance (streaming replies, in-memory
KB caching).
